...
exports.checkCoupon = async (req, res) => {
  ...
  if (req.session.coupon == undefined) {
    try {
      ...
      const { status, minimum_purchase, user_allowed, expiry, discount } =
        _coupon;
      if (status == 'listed' && user_allowed > 0 && expiry >= Date.now()) {
        if (minimum_purchase <= amount) {
          req.session.coupon = _coupon;
          reduceCoupon(_coupon._id);
          return res.status(200).json({ message: 'coupon applied', discount });
        }
...
async function reduceCoupon(id) {
  Coupon.updateOne({ _id: id }, { $inc: { user_allowed: -1 } }).exec(
    (err, result) => {
